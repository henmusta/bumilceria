...
<div class="card bg-light-info shadow-none position-relative overflow-hidden">
    <div class="row row-sm">
        <div class="col-lg-12 col-md-12">
            <form id="formUpdate" action="{{ route('backend.lb3jktpda.update', Request::segment(3)) }}">
                ...
                <div class="card-body">
                        ...
                        <div class="col-sm-6">
                            <div class="mb-3">
                               ...
                               <select id="select2Puskesmas" style="width: 100% !important;" name="updt_puskesmas_id">
                                <option value="{{ $data['lb3jktpda']['puskesmas']['id'] }}">{{$data['lb3jktpda']['puskesmas']['name'] }}</option>
                               </select>
                           </div>
                        </div>
                        <div class="col-sm-6">
                            <div class="mb-3">
                                ...
                                <select id="select2Datepicker" style="width: 100% !important;" name="tanggal">
                                  <option value="{{ $data['lb3jktpda']['tanggal'] }}">{{ \Carbon\Carbon::parse($data['lb3jktpda']['tanggal'])->isoFormat('MMMM YYYY') }}</option>
                                </select>
                            </div>
                        </div>
                    </div>
                    <div class="row">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control"  name="keterangan">{{ $data['lb3jktpda']['keterangan'] }}</textarea>
                        </div>
                    </div>
                </div>
  ...
  <script>
    $(document).ready(function () {
        ...
        const tableDetail = $('#DatatableDetail').DataTable({
			...
			ordering 	: false,
			info 		: false,
			data 		:  <?= isset($data['lb3jktpdadetail']) ? json_encode($data['lb3jktpdadetail']) : 'dummy' ;?>,
			columns : [
                {
					data 		: 'name',
					className 	: 'text-left',
